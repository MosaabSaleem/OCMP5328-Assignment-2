...
def load_model(path: str):
    ...
    if os.path.isfile(os.path.join(path, "adapter_config.json")):
        # LoRA adapter — load base first, then attach adapter
...
def generate(mdl, tok, prompt: str, max_new_tokens: int = 60) -> str:
...
def last_hidden(mdl, tok, text: str, max_length: int = 128):
    ...
    return out.hidden_states[-1].mean(dim=1).squeeze(0).cpu().numpy()